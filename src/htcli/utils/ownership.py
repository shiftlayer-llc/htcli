...
def user_owns_subnet(subnet_data: Dict[str, Any], user_addresses: List[Tuple[str, str]]) -> bool:
    """
...
def filter_user_assets(items: List[Dict[str, Any]], filter_field: str, user_addresses: List[Tuple[str, str]]) -> List[Dict[str, Any]]:
    """
...
def require_user_keys() -> List[Tuple[str, str]]:
    ...
    if not user_addresses:
        print_error("❌ No wallet keys found. The --mine filter requires stored wallet keys.")
        print_info("💡 Generate a key first:")
        print_info("   htcli wallet generate-key --name my-key")
        print_info("")
        print_info("💡 Or import an existing key:")
        print_info("   htcli wallet import-key --name my-key --mnemonic 'your seed phrase'")
        raise SystemExit(1)
        
    return user_addresses


def show_mine_filter_info(user_addresses: List[Tuple[str, str]], found_items: int, total_items: int = None):
    """
    ...
    """
    if found_items == 0:
        print_info(f"🔍 Filtered for your {len(user_addresses)} wallet address(es) - no matching assets found.")
        if total_items and total_items > 0:
            print_info(f"💡 Network has {total_items} total items, but none are owned by you.")
    else:
        print_info(f"🎯 Showing {found_items} asset(s) owned by your {len(user_addresses)} wallet address(es).")
        if total_items and total_items > found_items:
            print_info(f"💡 Network has {total_items} total items ({total_items - found_items} owned by others).")
