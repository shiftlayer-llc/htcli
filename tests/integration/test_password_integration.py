...
class TestPasswordIntegration:
    ...
    @pytest.mark.skip(reason="Environment variable persistence issue in test environment")
    def test_password_priority_order(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch('src.htcli.utils.password.get_stored_password') as mock_get_stored:
        ...
        with patch('src.htcli.utils.password.get_secure_password') as mock_get_password:
            ...
            with patch('src.htcli.dependencies.get_client') as mock_get_client:
                ...
                result = cli_runner.invoke(app, [
                    ...
                    '--delegate-stake-percentage', '10',
                    '--churn-limit', '5',
                    '--registration-queue-epochs', '10',
                    '--activation-grace-epochs', '5',
                    '--queue-classification-epochs', '3',
                    '--included-classification-epochs', '2',
                    '--max-registered-nodes', '100',
                    '--initial-coldkeys', '5GrwvaEF5zXb26Fz9rcQpDWS57CtERHpNehXCPcNoHGKutQY',
                    '--key-types', 'sr25519',
                    '--node-removal-system', 'manual',
                    '--key-name', 'test-key'
                ])
