...
def test_wallet_cli_create(tmp_path):
    def with_password(password):
        ...
        result = subprocess.run([
            sys.executable, "-m", "htcli.main", "wallet", "create", "--name", "cli_test_wallet", "--path", str(tmp_path), "--password", password
    ...
    def without_password():
        ...
        result = subprocess.run([
            sys.executable, "-m", "htcli.main", "wallet", "create", "--name", "cli_test_wallet", "--path", str(tmp_path), "--password", "", "--force"
        ], capture_output=True, text=True)
        assert result.returncode == 0
        assert "Created wallet" in result.stdout
        return result.stdout
